...
describe("DisplaySpecBuilder.buildToolSpec", () => {
  ...
  describe("title generation for different tool kinds", () => {
    ...
    it("codesearch title includes query and path when kind is other", () => {
      const entry = makeEntry({
        name: "codesearch",
        kind: "other",
        rawInput: { query: "ToolCard", path: "/repo/src" },
      });
      const spec = builder.buildToolSpec(entry, "medium");
      expect(spec.title).toBe("Code search \"ToolCard\" in /repo/src");
    });
    ...
    it("uses working-directory relative path for read/write/apply_patch", () => {
      ...
      expect(readSpec.title).toBe("src/a.ts (lines 10–14)");
      ...
      expect(writeSpec.title).toBe("lib/output.json");
      ...
      expect(patchSpec.title).toBe("packages/a/src/index.ts");
    });

    it("apply_patch multi-file title shows first two relative paths", () => {
      ...
      expect(spec.title).toBe("a/one.ts, b/two.ts (+1 files)");
    });

    it("uses working-directory relative path for codesearch path", () => {
      const spec = builder.buildToolSpec(
        makeEntry({
          name: "codesearch",
          kind: "other",
          rawInput: { query: "Session", path: "/repo/src/core" },
        }),
        "medium",
        { id: "sess-1", workingDirectory: "/repo" },
      );
      expect(spec.title).toBe("Code search \"Session\" in src/core");
    });
  });
});
